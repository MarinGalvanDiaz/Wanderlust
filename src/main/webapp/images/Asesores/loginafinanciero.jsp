<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="shortcut icon" href="imgasesores/logo_sin_fondo.png" type="image/x-icon">
    <title> WANDERLUST </title>
    <link rel="stylesheet" href="loginafinanciero.css">
</head>
<body>

    <div class="login-box">
        <img class="avatar" src="imgasesores/logo_sin_fondo.png" alt="WANDERLUST">
        <h1> LOGIN HERE </h1>

        <form action="">
            <!-- USERNAME-->
            <label for="username"> Username </label>
            <input type="text" placeholder="Ingrese su Username" name="usuario">
    
            <!-- ID / PASSWORD-->
            <label for="password"> Password </label>
            <input type="password" placeholder="Ingrese su password" name="contrasenia">
   
            <!-- NUMERO DE TELEFONO -->
            <label for="numerote"> Número telefónico </label>
            <input type="text" placeholder="Ingrese su numero telefonico" name="numerotele">

            <!-- EMAIL -->
            <label for="email"> E-mail </label>
            <input type="text" placeholder="Ingrese su correo electronico" name="email">
        
            <input type="submit"  name="altas" value="Ingresar">

            <a href=""> Perdiste tu password? </a>
            <br>
            <a href=""> Mas informacion </a>
            <div class="regresar">
                <a href="asesores.html"> Regresar </a>
            </div>

        </form>  

    </div>

    <%
            String UsarioA=request.getParameter("usuario");
            String Contra = request.getParameter("contrasenia");
            String Numte = request.getParameter("numerotele");
            String Correoel = request.getParameter("email");
           

            Connection conx = null;
            Statement sta = null;

            try {
                Class.forName("com.mysql.jdbc.Driver");
                conx = DriverManager.getConnection("jdbc:mysql://localhost/wanderlust?autoReconnect=true&useSSL=false", "root", "n0m3l0");
                sta = conx.createStatement();
            } catch (SQLException error) {
                out.print(error.toString());
            }

            try {
                sta.executeUpdate("insert into asesorfinanciero values('" + UsuarioA + "','" + Contra + "','" + Numte+ "','" + Correoel + "');");
                out.println("<script> alert('Ingreso exitoso')</script>");
                conx.close();
                sta.close();
            } catch (SQLException error) {
                out.print(error.toString());
            }
        %>
</body>
</html>